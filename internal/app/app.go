...
func runTelegram(
	...
	saveService, err := buildSaveService(ctx, cfg)
	if err != nil {
		return err
	}
	...
	handler := telegram.NewCommandHandler(telegram.CommandHandlerOptions{
		...
		LogPath: cfg.LogPath,
		Schedule: telegram.ScheduleInfo{
			Timezone: cfg.Schedule.Timezone,
			SyncCron: cfg.Schedule.AutoSyncCron,
			PushCron: cfg.Schedule.AutoPushCron,
			LogsCron: cfg.Schedule.AutoLogsCron,
		},
